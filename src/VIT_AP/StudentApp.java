...
package VIT_AP;
import java.sql.*;
import application.CourseEnrollment;
...
public class StudentApp extends Application {
	...
	public void Student(String user,String ID,Stage primaryStage)
	{
		this.user=user;
		this.ID=ID;
		start(primaryStage);
    ...
    private void performAction(String selectedOption) {
        switch (selectedOption) {
            case "Course Enrollment":
            	try {
            	CourseEnrollment CE=new CourseEnrollment("Student",ID);
				CE.start(primaryStage);
            	}
            	catch(Exception e)
                {
                	showAlert(Alert.AlertType.ERROR, "Form Error!",
            ...
            case "View my Attendance":
            	ViewStudAttendance VA = new ViewStudAttendance();
            	VA.View(user,ID,primaryStage);
                break;
                
            case "View my Registration Details":
            	View_Details VMD=new View_Details("Student",ID);
				VMD.start(primaryStage);
                break;
            case "View my Class Details":
            	Class_Details VCD=new Class_Details("Student",ID);
    ...
    private static Stage getStage() {
        return (Stage) StudentApp.getStage().getScene().getWindow();
    }
